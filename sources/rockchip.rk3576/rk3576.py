...
verify/symbols-0824/review/rk3576.md §7 V2). GND: twelve pure single-column units of the DS
returns (VSS_0..162, AVSS_0..76, AVSS1_0..24 = 163+77+25 = 265 balls; TVSS, PLL_AVSS and DDRPHY_PLL_AVSS
ride in units E/C with the domain they return) — 10 x 24 + 13 + 12 so no 1-pin box (D9).
...
def _gnd_columns():
    """265 DS ground returns in vendor order, chunked 10 x 24 + 13 + 12 (never a 1-pin box)."""
    from symlib.rk3576_facts import ball_of
    balls = [ball_of(n) for n in _GND_ORDER]
    assert len(balls) == 265
    sizes = [GND_PER_UNIT] * 10 + [13, 12]
    out, i = [], 0
    for n in sizes:
        out.append(balls[i:i + n])
        i += n
    assert i == 265 and all(len(c) <= GND_PER_UNIT for c in out)
    return out
...
UNIT_BALLS.update({UNIT[f"GND{k + 1}"]: list(c) for k, c in enumerate(GND_COLUMNS)})
...
def rk3576():
    ...
    for k, col in enumerate(GND_COLUMNS):
        # width=25.4: a caption-less single left column needs no 50.8 body floor — the widest
        # name (AVSS1_24, ~8.4 mm ink) fits; the narrow body keeps 12 columns on one A0 row.
        units.append(Unit(UNIT_TITLE[UNIT[f"GND{k + 1}"]],
                          left=[Group("", [_pin(b) for b in col])], width=25.4))
    assert len(units) == N_UNITS == 34
